...
const applyAGCSettings = (settings) => {
	const prefix = "MMAUDIO|AGC settings|";

	sendCat(`${prefix}Threshold S=${settings.thresholdS};`, false);
	sendCat(`${prefix}Slope dB per dB=${settings.slopeDbPerDb};`, false);
	sendCat(`${prefix}Noise filter=${settings.noiseFilter};`, false);
	sendCat(`${prefix}Hang time=${settings.hangTime};`, false);
	sendCat(`${prefix}Smooth samples=${settings.smoothSamples};`, false);
	sendCat(`${prefix}Recovery db/s=${settings.recoveryDbPerSecond};`, false);
	sendCat(`${prefix}Sample blocks=${settings.sampleBlocks};`, false);
};
